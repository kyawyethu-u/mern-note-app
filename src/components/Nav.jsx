  ...
  return (
    <nav className='bg-slate-50 py-4 px-10 font-mono'>
        <div className='flex items-center justify-between'>
          <Link to={"/"} className='text-teal-600 font-bold text-4xl'>Denote.io</Link>
        <div className='flex gap-3'>
          {
            token ? <>
            <Link to={"/create"} className='text-teal-600 font-medium'>Create Note</Link> 
        ...
        </div>
          {
            token && token.user_mail && (
              <p className='text-right text-sm text-teal-600 '>
                <span className='font-semibold'>Login as </span>{token.user_mail}</p>
            )
          }
    </nav>
  )
